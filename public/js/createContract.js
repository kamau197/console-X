...
async function triggerEmailOwnershipCheck(email) {
  ...
  setTimeout(() => {
    ...
    qs('#emailVerificationStatus').innerText = "Verified ✔";
  }, 3000);
}
...
function addMilestone({title='New milestone',extra='',locked=false}={}) {
  ...
  // Place before "Complete contract milestone"
  const items=[...milestoneList.querySelectorAll('.ms-item')];
  const complete=items.find(i=>(i.querySelector('.ms-title')?.value||'').toLowerCase().includes('complete contract'));
  if(complete) milestoneList.insertBefore(div, complete);
  else milestoneList.appendChild(div);
...
function addBaseMilestones(type){
  ...
  if(type==='GA'){
    addMilestone({title:'Party B makes upfront payment',extra:'<button class="btn ghost sendLinkBtn">Send Payment Link</button>',locked:true});
    addMilestone({
      title:'Grant access milestone',
      extra:`<div>Email to grant access: <strong>${partyAEmailField.value}</strong></div>
    ...
    addMilestone({title:'Pay upfront milestone',extra:'<button class="btn ghost makePaymentBtn">Make Payment</button>',locked:true});
...
function renderPreview(){
  const A = qs('#partyA').value || '[Party A]',
        ...
        ms = gatherMilestones(),
        appId = appIdField.value || 'N/A';  // <-- NEW
  ...
  qs('#previewContent').innerHTML=`
    ...
    <strong>App ID:</strong> ${appId}<br><br>
...
function addBaseMilestones(type){
  ...
  // --- Default milestones for each contract type ---
  if(type==='GA'){
    addMilestone({title:'Party B makes upfront payment',extra:'<button class="btn ghost sendLinkBtn">Send Payment Link</button>',locked:true});
  ...
  // --- NEW: Insert App Is Live milestone ---
  addMilestone({
    title: "App Is Live",
    extra: "<div class='small'>(System Controlled)</div>",
    locked: true
  });

  // --- Final Complete Contract milestone ---
  addMilestone({
